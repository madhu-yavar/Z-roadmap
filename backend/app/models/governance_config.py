...
class GovernanceConfig(Base):
    ...
    team_fs: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    ...
    roadmap_locked: Mapped[bool] = mapped_column(default=False, nullable=False)
    roadmap_locked_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    roadmap_locked_by: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    roadmap_lock_note: Mapped[str] = mapped_column(default="", nullable=False)
